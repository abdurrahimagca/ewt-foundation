...
const props = defineProps<{
  initialProduct: Entity<"product">[] | null;
  limit: number;
}>();

const emit = defineEmits<{
  (e: "handle:commitChanges", value: Entity<"product">[]): void;
}>();
...
function addToSelecteds(product: Entity<"product">) {
  ...
  if (selecteds.value?.length >= props.limit) {
    notification.dispatch({
      title: "Error",
      message: `You can only select ${props.limit} products`,
    });
    return;
  }
...
function commitChanges() {
  ...
  emit("handle:commitChanges", selecteds.value);
